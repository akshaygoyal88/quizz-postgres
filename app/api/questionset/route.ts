...
import {
  createQuestionSet,
  getQuizzesWithPaginationByCreatedBy,
  getQuizzesByCreatedBy,
...
export async function GET(req: Request) {
  ...
  try {
    if (page > 0 && pageSize > 0 && createdById) {
      ...
      const allQuestionSets = createdById
        ? await getQuizzesByCreatedBy(createdById)
        : await getQuizzesByCreatedBy();
...
export async function POST(req: any, res: any) {
  try {
    ...
    const createSet = await createQuestionSet({
      createdById,
      name,
      description,
    });
