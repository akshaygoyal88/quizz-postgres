...
export async function handleProfileSubmit(
  ...
  if (missingFields.length > 0) {
    ...
    typeof rawFormData.pincode !== "string" ||
    rawFormData.pincode.length < 6
  ) {
    ...
    result = {
      error: { pincode: "Pincode should not be more than 6 characters." },
    };
  } else if (
    typeof rawFormData.mobile_number !== "string" ||
    rawFormData.mobile_number.length <= 15
  ) {
    result = {
      error: { mobile_number: "Phone number should not be less than 10." },
    };
  } else if (
    typeof rawFormData.mobile_number !== "string" ||
    rawFormData.mobile_number.length > 16
  ) {
    result = {
      error: { mobile_number: "Phone number should not be more than 10." },
    };
  } else {
